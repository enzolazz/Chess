...
Game::Game(sf::RenderWindow &window, float squareSize)
    : _window(window), squareSize(squareSize) {
...
void Game::pieceReleased() {
  ...
  if (!legalMove)
    ...
    move_tuple move(moving, oldSquare,
                    board->getPiece(movingSquare.x, movingSquare.y),
                    movingSquare, board->getOrientation());
    moves.push(move);
...
bool Game::isLegalMove(Piece *piece, Square *square) {
  ...
  case 'p':
    ...
    if (dX != 0 && (abs(dY) != 1 || newSquarePiece == nullptr ||
  ...
  case 'k':
    ...
    if (rook != nullptr) {
      ...
      move_tuple rookCastled(rook, rook->getSquare(), nullptr, castledSquare,
...
bool Game::turnCheck(char pieceType) {
  return (whiteTurn && std::isupper(pieceType)) ||
         (!whiteTurn && std::islower(pieceType));
}
...
void Game::showAvailableSquares() {
  ...
  for (Square square : moves) {
    ...
    sf::Vector2f pos(square.x * square.size + square.size / 2,
                     square.y * square.size + square.size / 2);
